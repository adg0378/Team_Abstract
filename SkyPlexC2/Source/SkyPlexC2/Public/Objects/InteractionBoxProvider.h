...
#include "InteractionBoxProvider.generated.h"
...
enum class EKeyValDisplayType : uint8
{
	...
	POIParams UMETA(DisplayName = "POIParams"),
	AOIParams UMETA(DisplayName = "AOIParams"),
	TakeoffParams UMETA(DisplayName = "TakeoffParams"),
	GeoFenceParams UMETA(DisplayName = "GeoFenceParams")
};
...
class UInteractionBoxProvider : public UInterface
{
	GENERATED_BODY()
};
...
**/
class SKYPLEXC2_API IInteractionBoxProvider
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable)
	void ToggleCull(bool IsCulled, bool FromFlyTo);

	UFUNCTION(BlueprintNativeEvent, BlueprintCallable)
	void SetLinkedProvider(const TScriptInterface<UInteractionBoxProvider>& InProvider);

	UFUNCTION(BlueprintNativeEvent, BlueprintCallable)
	void GetLinkedProvider(TScriptInterface<UInteractionBoxProvider>& OutProvider) const;
