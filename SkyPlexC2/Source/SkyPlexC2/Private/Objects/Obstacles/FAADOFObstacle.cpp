...
void AFAADOFObstacle::SetData(FFAAObjectDataStruct InData) {
	Data = InData;
	HeightAboveGroundLevelMeters = InData.agl * 0.3048;
	OnSetData();
}

void AFAADOFObstacle::GetData(FFAAObjectDataStruct& OutData) {
	OutData = Data;
}

void AFAADOFObstacle::OnSetData_Implementation() {}

void AFAADOFObstacle::GetInteractionBoxTitle_Implementation(FText& OutTitle) {
	OutTitle = FText::FromString(Data.oas);
}

void AFAADOFObstacle::GetInteractionBoxKeyVals_Implementation(TMap<FString, FInteractionBoxValue>& OutKeyVals) {
