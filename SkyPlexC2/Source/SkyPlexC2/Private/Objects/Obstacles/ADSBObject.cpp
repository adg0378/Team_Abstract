...
void AADSBObject::ToggleCull_Implementation(bool IsCulled, bool FromFlyTo) {
...
void AADSBObject::SetTrailLength(int Length) {
    Trail->SetTrailLimit(Length);
}

void AADSBObject::SetData(const FADSBAircraftStruct& InData) {
    Data = InData;
    ...
    this->cesiumGlobeAnchor->SetEastSouthUpRotation(FQuat::MakeFromEuler(FVector(Data.roll, Pitch, Yaw)));
...
void AADSBObject::GetData(FADSBAircraftStruct& OutData) const {
    OutData = Data;
}

void AADSBObject::GetInteractionBoxTitle_Implementation(FText& OutTitle) {
    OutTitle = FText::FromString(FString::Printf(TEXT("%s"), *Data.flight));
}

void AADSBObject::GetInteractionBoxKeyVals_Implementation(TMap<FString, FInteractionBoxValue>& OutKeyVals) {
