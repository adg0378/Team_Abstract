...
#include "Player/CesiumSpringArmComponent.h"
#include "CesiumGlobeAnchorComponent.h"
#include "CesiumGeoreference.h"
#include "PhysicsEngine/PhysicsSettings.h"

void UCesiumSpringArmComponent::OnRegister() {
	this->ResolveOriginShift();
	Super::OnRegister();
}

void UCesiumSpringArmComponent::BeginPlay() {
	this->ResolveOriginShift();
	Super::BeginPlay();
}

void UCesiumSpringArmComponent::ResolveOriginShift() {
	this->OriginShift = nullptr;
	...
	this->OriginShift =
		Owner->FindComponentByClass<UCesiumOriginShiftComponent>();
	if (!IsValid(this->OriginShift)) {
		// A globe anchor is missing and required, so add one.
		this->OriginShift =
			Cast<UCesiumOriginShiftComponent>(Owner->AddComponentByClass(
				UCesiumOriginShiftComponent::StaticClass(),
				false,
				FTransform::Identity,
				false));
		Owner->AddInstanceComponent(this->OriginShift);
	}
}

void UCesiumSpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime) {
	...
	UCesiumGlobeAnchorComponent* GlobeAnchor = this->OriginShift->GetGlobeAnchor();
	if (!GlobeAnchor) {
		return;
	}

	ACesiumGeoreference* Georeference = GlobeAnchor->GetResolvedGeoreference();
	if (!Georeference) {
		return;
	}
	...
	PreviousDesiredLoc = Georeference->TransformEarthCenteredEarthFixedPositionToUnreal(PreviousDesiredLocECEF);
	...
	PreviousDesiredLocECEF = Georeference->TransformUnrealPositionToEarthCenteredEarthFixed(DesiredLoc);
