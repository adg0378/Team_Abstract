...
void UGeoClusterer::SetClusterRestrictions(float InMaxClusterRadiusNM, float InPointBufferNM, float InInitialClusterRadiusNM) {
	MaxClusterRadiusNM = InMaxClusterRadiusNM;
	PointBufferNM = InPointBufferNM;
	InitialClusterRadiusNM = InInitialClusterRadiusNM;
}

void UGeoClusterer::GetClusters(TMap<FGuid, FClusterData>& OutClusters) const {
	OutClusters = Clusters;
}

void UGeoClusterer::SetClusterUpdatedCallback(FClusterUpdatedCallback InCallback) {
	OnClusterUpdated = InCallback;
}

void UGeoClusterer::SetClusterRemovedCallback(FClusterRemovedCallback InCallback) {
	OnClusterRemoved = InCallback;
}

void UGeoClusterer::SetClusterMergedCallback(FClusterMergedCallback InCallback) {
	OnClusterMerged = InCallback;
}

void UGeoClusterer::ToggleExternalUpdates(bool InProvideUpdates) {
	ProvideUpdates = InProvideUpdates;
}

void UGeoClusterer::RefreshClusters() {
	if (ProvideUpdates) {
...
void UGeoClusterer::GetClusterIDAtLocation(const FVector& InLonLatHeight, TSet<FGuid>& OutClusterIDs) {
	OutClusterIDs.Empty();
...
void UGeoClusterer::AddPoint(const FString& Name, const FVector& LonLatHeight) {
...
void UGeoClusterer::AddCluster(FPointData& InitialPoint) {
...
void UGeoClusterer::RemoveCluster(const FGuid& ClusterID) {
...
void UGeoClusterer::MovePoint(const FString& PointName, const FVector& NewLonLatHeight) {
...
void UGeoClusterer::RemovePoint(const FString& PointName, bool KeepClusterIfEmpty) {
	FPointData* Point = Points.Find(PointName);
	if (!Point) {
		return;
	}
...
void UGeoClusterer::UpdateCluster(const FGuid& ClusterID) {
	FClusterData* Cluster = Clusters.Find(ClusterID);
	if (!Cluster) {
		return;
	}
...
void UGeoClusterer::MergeClusters(const FGuid& ClusterAID, const FGuid& ClusterBID) {
...
float UGeoClusterer::DistanceNM(const FVector& LonLatHeightA, const FVector& LonLatHeightB) {
