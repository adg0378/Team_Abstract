...
void ASPFlightTrail::ResetTrail() {
	TrailMesh->ClearMeshSection(0);
	if (TrailPoints.Num() > 0) {
		GlobeAnchor->MoveToLongitudeLatitudeHeight(LonLatHeights[0]);
	}

	TrailPoints.Reset();
	
	for (const FVector& LLH : LonLatHeights) {
		TrailPoints.Add(Georeference->TransformLongitudeLatitudeHeightPositionToUnreal(LLH) - GetActorLocation());
	}
...
void ASPFlightTrail::AddTrailPointCoord(const FVector& LonLatHeight) {
	if (LonLatHeight == FVector(0.0, 0.0, 0.0)) {
		// prevents ccsim drone from ros2 repo not hooked up to px4 yet from creating first point way far away from where it actually spawns
		return;
	}
	...
	if (TrailPoints.Num() == 0) {
		GlobeAnchor->MoveToLongitudeLatitudeHeight(LonLatHeight);
	}

	if (TrailLimit > 0 && TrailPoints.Num() + 1 > TrailLimit) {
		TrailPoints.RemoveAt(0, EAllowShrinking::No);
		LonLatHeights.RemoveAt(0, EAllowShrinking::No);
	}

	LonLatHeights.Add(LonLatHeight);
	TrailPoints.Add(Georeference->TransformLongitudeLatitudeHeightPositionToUnreal(LonLatHeight) - GetActorLocation());
	//DrawDebugPoint(GetWorld(), GetActorLocation(), 10.0f, FColor::Red, false, 5.0f);
	UpdateTrailMesh();
}
