...
void USPDroneManager::setSelectedDroneId(int32 DroneID)
{
    selectedDroneId = DroneID;
}

ABasicDrone *USPDroneManager::getSelectedDrone()
{
    ...
    for (const TPair<int32, FSPSwarmStruct> &Pair : Drones)
    {
        ...
        const TObjectPtr<ABasicDrone> *FoundDrone = Pair.Value.Drones.Find(selectedDroneId);
        if (FoundDrone)
        {
            return FoundDrone->Get();
        }
    }
    return nullptr;
}
